...
def calc_historic(date, team, opp):
    ...
    fixtures = json.loads(connection.getresponse().read().decode())['fixtures']
    ...
    if is_team_home is True:
        ...
        our_team += 10 * int(int(team_data["statistics"][0]["wins_home"]) / (int(team_data["statistics"][0]["wins_home"])+int(team_data["statistics"][0]["draws_home"])+int(team_data["statistics"][0]["losses_home"])))
        opp_team += 10 * int(int(opp_data["statistics"][0]["wins_away"]) / (int(opp_data["statistics"][0]["wins_away"])+int(opp_data["statistics"][0]["draws_away"])+int(opp_data["statistics"][0]["losses_away"])))
    else:
        # opp is home, team is away
        our_team += 10 * int(int(team_data["statistics"][0]["wins_away"]) / (int(team_data["statistics"][0]["wins_away"])+int(team_data["statistics"][0]["draws_away"])+int(team_data["statistics"][0]["losses_away"])))
        opp_team += 10 * int(int(opp_data["statistics"][0]["wins_home"]) / (int(opp_data["statistics"][0]["wins_home"])+int(opp_data["statistics"][0]["draws_home"])+int(opp_data["statistics"][0]["losses_home"])))
    ...
    if len(h2h) > 1:
        ...
        our_score0, opp_score0 = util.calc_h2h_points(winner0, h2h, 0, team_id_c)
        our_score1, opp_score1 = util.calc_h2h_points(winner1, h2h, 1, team_id_c)
        our_team += our_score0 + our_score1
        opp_team += opp_score0 + opp_score1
    elif len(h2h) > 0:
        winner = h2h[0]['outcome']['winner']
        our_score, opp_score = util.calc_h2h_points(winner, h2h, 0, team_id_c)
        our_team += our_score
        opp_team += opp_score
    else:
        our_team += 0
